  ...
  const checkConditionDetail = (job, conditionId) => {
    switch(conditionId) {
      ...
      case 'commuteMethod':
        ...
        import { Plus, Trash2, Edit2, Save, X, Download, CheckCircle, Heart, History, Phone, ChevronDown, ChevronRight, User, Target, ZoomIn, ZoomOut, Maximize2, AlertCircle, Check, Loader, XCircle, MinusCircle } from 'lucide-react';
        ...
        const Toast = ({ message, type = 'success', onClose }) => {
          ...
          return (
            <div className={`fixed top-4 right-4 ${bgColor} text-white px-6 py-3 rounded-lg shadow-xl z-50 flex items-center space-x-2 animate-slide-in`}>
        ...
        const LoadingSpinner = () => (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-8 flex flex-col items-center space-y-4">
              <Loader className="animate-spin text-indigo-600" size={48} />
              <p className="text-gray-700 font-medium text-lg">分析中...</p>
            </div>
          </div>
        );
        ...
        const JobMatchingFlowchart = () => {
          ...
          // ノードの色を取得する関数
          const getNodeColors = (node, isOnTrackingPath) => {
            const colorSchemes = {
              start: {
                bg: 'bg-indigo-50',
          ...
          const renderTreeNode = (node) => {
            ...
            const colors = getNodeColors(node, isOnTrackingPath);
            ...
            return (
              <div key={node.id}>
                <div
                  className={`absolute ${colors.bg} border-2 ${borderColor} rounded-lg shadow-md hover:shadow-xl transition-all duration-200 cursor-pointer ${
                  ...
                  <div className="p-3">
                    ...
                    {node.type === 'success' && (
                      <div className="text-center">
                        {/* ヘッダー */}
                        <div className={`${colors.header} -mx-3 -mt-3 px-3 py-3 mb-3 rounded-t-lg`}>
                          <div className="flex items-center justify-center space-x-2">
                            <CheckCircle className={colors.icon} size={ICON_SIZES.sm} />
                            <span className="text-sm font-bold text-emerald-900">紹介可能</span>
                          </div>
                          <div className="text-sm text-emerald-700 mt-1 font-semibold">
                            {node.jobs.length}件の案件
                          </div>
                        </div>
                        
                        {/* 案件リスト */}
                        <div className="space-y-2 max-h-44 overflow-y-auto custom-scrollbar">
                          {node.jobs.slice(0, 3).map(job => (
                            <div key={job.id} className="bg-emerald-50 rounded-lg p-2.5 border border-emerald-200 hover:border-emerald-400 transition">
                              <div className="font-semibold text-sm text-gray-900 truncate mb-1.5" title={job.name}>
                                {job.name}
                              </div>
                              {job.fee && (
                    ...
                    {node.type === 'fail' && (
                      <div className="text-center py-2">
                        <MinusCircle className={colors.icon} size={ICON_SIZES.md} />
                        <div className="text-sm font-bold text-gray-700 mt-2">提案失敗</div>
                      </div>
                    )}
                    ...
                    {node.type !== 'success' && node.type !== 'fail' && (
                      <>
                        {/* ヘッダー */}
                        <div className={`${colors.header} -mx-3 -mt-3 px-3 py-2.5 mb-3 rounded-t-lg border-b-2 ${borderColor}`}>
                          <div className={`text-sm font-bold text-center ${colors.icon} flex items-center justify-center space-x-2`}>
                            {node.type === 'start' && <Target size={ICON_SIZES.sm} />}
                            {node.type === 'pass' && <CheckCircle size={ICON_SIZES.sm} />}
                            {node.type === 'relax' && <AlertCircle size={ICON_SIZES.sm} />}
                            {node.type === 'relax-accepted' && <Check size={ICON_SIZES.sm} />}
                            {node.type === 'relax-rejected' && <X size={ICON_SIZES.sm} />}
                            {node.type === 'exclude' && <XCircle size={ICON_SIZES.sm} />}
                            <span>{node.condition || '開始'}</span>
                          </div>
                        </div>
                          ...
                          <>
                            ...
                            <div className="mb-3 pb-3 border-b border-gray-200">
                              <div className="flex items-center justify-between text-sm">
                                ...
                                <span className="font-bold text-indigo-600">{node.jobs.length}件</span>
                              </div>
                              {fees.length > 0 && (
                                <div className="flex items-center justify-between text-sm mt-1.5">
                                  <span className="text-gray-600">Fee</span>
                                  <span className="font-bold text-emerald-600">
                                    {minFee === maxFee ? `${minFee}万` : `${minFee}-${maxFee}万`}
                                  </span>
                                </div>
                              )}
                            </div>
                            ...
                            <div className="space-y-2">
                              {node.jobs.slice(0, 3).map(job => (
                                <div key={job.id} className="flex items-center justify-between text-sm p-2 bg-gray-50 rounded hover:bg-gray-100 transition">
                                  <div className="flex-1 truncate font-medium text-gray-800 mr-2" title={job.name}>
                                    {job.name}
                                  </div>
                                  {job.fee && (
                              ...
                              {node.jobs.length > 3 && (
                                <div className="text-sm text-center text-gray-500 py-1">
                                  +{node.jobs.length - 3}件
                                </div>
                              )}
                            </div>
                          </>
                ...
                {hoveredNode === node.id && node.jobs.length > 0 && (
                  <div
                    className="absolute bg-white border-2 border-indigo-500 rounded-xl shadow-2xl p-4 z-50"
                    style={{
                      left: `${pos.x + 240}px`,
                      top: `${pos.y}px`,
                      width: '320px',
                  ...
                  >
                    {/* ヘッダー */}
                    <div className="flex items-center justify-between mb-3 pb-3 border-b-2 border-indigo-200">
                      <div className="flex items-center space-x-2">
                        <Target className="text-indigo-600" size={ICON_SIZES.md} />
                        <h3 className="font-bold text-indigo-900 text-base">全案件リスト</h3>
                      </div>
                    ...
                    {/* Fee統計 */}
                    {fees.length > 0 && (
                      <div className="bg-gradient-to-r from-emerald-50 to-emerald-100 rounded-lg p-3 mb-3">
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-sm text-emerald-700 font-semibold">Fee範囲</span>
                          <span className="text-base font-bold text-emerald-800">
                            {minFee === maxFee ? `${minFee}万円` : `${minFee}-${maxFee}万円`}
                          </span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-sm text-emerald-700 font-semibold">平均Fee</span>
                    ...
                    <div className="space-y-2 max-h-80 overflow-y-auto custom-scrollbar">
                      {node.jobs.map(job => (
                        <div key={job.id} className="bg-gray-50 rounded-lg p-3 border border-gray-200 hover:border-indigo-400 hover:shadow-md transition">
                          <div className="flex items-start justify-between mb-2">
                            <div className="font-semibold text-sm text-gray-900 flex-1 mr-2">
                              {job.name}
                            </div>
                            <span className={`text-sm font-bold px-2 py-1 rounded whitespace-nowrap ${
                              parseInt(job.fee) >= 35 
                                ? 'bg-amber-100 text-amber-800' 
                                : parseInt(job.fee) <= 20 
                                ? 'bg-gray-200 text-gray-600' 
                          ...
                          <div className="flex items-center justify-between text-sm text-gray-600">
                            <span>スコア: <span className="font-bold text-indigo-600">{calculateMatchScore(job)}</span></span>
            ...
            <div className="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-lg shadow-lg p-4 text-white">
              <div className="mb-4">
                <h3 className="font-bold text-base md:text-lg mb-3 flex items-center">
                  <User className="mr-2" size={ICON_SIZES.md} />
                  👤 求職者の基本情報
                </h3>
              ...
              <div className="border-t border-white border-opacity-30 pt-4">
                <h3 className="font-bold text-base md:text-lg mb-3 flex items-center">
                  <Target className="mr-2" size={ICON_SIZES.md} />
                  📊 マッチング状況
                </h3>
                <div className="grid grid-cols-3 gap-3 text-xs md:text-sm">
                  <div className="bg-emerald-500 bg-opacity-40 rounded p-3 text-center hover:bg-opacity-50 transition">
                    <div className="font-bold text-3xl md:text-4xl">{immediateMatches.length}</div>
                    <div className="text-xs md:text-sm mt-1">すぐ紹介可能</div>
                  </div>
                  <div className="bg-amber-500 bg-opacity-40 rounded p-3 text-center hover:bg-opacity-50 transition">
                ...
                {!showAnalysis ? (
                  <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        <div>
                          ...
                          <input
                            ...
                            className={`w-full px-4 py-3 text-sm border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition ${
                        ...
                        <div>
                          ...
                          <select
                            ...
                            className="w-full px-4 py-3 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition"
                            aria-label="性別を選択"
                          >
                      ...
                      <div className="mt-6 border border-indigo-200 rounded-lg overflow-hidden">
                        ...
                        >
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            {showPrioritySettings ? (
                              <ChevronDown className="text-indigo-600" size={ICON_SIZES.md} />
                            ) : (
                              <ChevronRight className="text-indigo-600" size={ICON_SIZES.md} />
                            )}
                          </div>
                        </button>
                      ...
                      {jobs.length > 0 && (
                        <div className="mt-4 p-3 bg-emerald-50 border border-emerald-300 rounded-lg">
                          <p className="text-emerald-700 text-sm font-medium flex items-center">
                            <CheckCircle className="mr-2" size={ICON_SIZES.sm} />
                    ...
                    <div className="bg-white rounded-lg shadow-lg p-4 md:p-6">
                      ...
                      <button
                        onClick={addJob}
                        className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-6 rounded-lg flex items-center justify-center space-x-2 transition-all duration-200 transform hover:scale-105 focus:ring-4 focus:ring-indigo-300 shadow-md"
                        aria-label="案件を追加"
                      >
                                ...
                                {editingJobId === job.id ? (
                                  <div className="bg-blue-50 p-4 rounded-lg border-2 border-blue-300">
                                    ...
                                    <div className="flex items-center space-x-2">
                                      <button
                                        onClick={saveEditJob}
                                        className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white py-2 px-4 rounded-lg transition text-sm flex items-center justify-center space-x-1"
                                        aria-label="変更を保存"
                                      >
                                  ...
                                  <div className={`bg-gray-50 p-4 rounded-lg hover:bg-gray-100 transition border-2 ${
                                    ...
                                    <div className="flex items-center justify-between flex-wrap gap-2">
                                      ...
                                      <div className="flex items-center space-x-1">
                                        ...
                                        >
                                          <Heart size={ICON_SIZES.md} fill={isFavorite ? 'currentColor' : 'none'} />
                                        </button>
                                        <button
                                          onClick={() => startEditJob(job)}
                                          className="text-blue-600 p-2 hover:bg-blue-100 rounded-lg transition-all duration-200 hover:scale-110"
                                          aria-label="案件を編集"
                                        >
                                        ...
                                        <button
                                          ...
                                          className="text-red-600 p-2 hover:bg-red-100 rounded-lg transition-all duration-200 hover:scale-110"
                                          aria-label="案件を削除"
                                        >
                            ...
                            return (
                              <div key={job.id} className="border-2 border-purple-300 rounded-lg p-4 bg-purple-50 hover:shadow-lg transition">
                                ...
                                <div className="space-y-2 text-xs md:text-sm">
                                  ...
                                  <div className="flex justify-between py-1 border-b border-purple-200">
                                    <span className="text-gray-600">Fee:</span>
                                    <span className="font-bold text-emerald-600">{job.fee}万</span>
                                  </div>
                                  <div className="flex justify-between py-1 border-b border-purple-200">
                    ...
                    {searchHistory.length > 0 && (
                      <div className="bg-white rounded-lg shadow-lg p-4 md:p-6">
                        <h2 className="text-lg md:text-xl font-bold text-indigo-600 mb-3 flex items-center">
                          <History className="mr-2" size={ICON_SIZES.md} />
                          検索履歴
                        </h2>
                        <div className="space-y-2">
                    ...
                    {jobs.length > 0 && (
                      <div className="text-center">
                        <button
                          ...
                          className="bg-gradient-to-r from-emerald-600 to-emerald-600 hover:from-emerald-700 hover:to-emerald-700 text-white font-bold py-4 px-8 md:px-12 rounded-lg text-base md:text-lg shadow-lg transition-all duration-200 transform hover:scale-105 focus:ring-4 focus:ring-emerald-300"
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center flex-wrap gap-2">
                      <button
                        ...
                        className="bg-gray-600 hover:bg-gray-700 text-white font-bold py-2 px-4 md:px-6 rounded-lg transition-all duration-200 text-sm md:text-base focus:ring-4 focus:ring-gray-300 shadow-md"
                        aria-label="入力画面に戻る"
                      >
                        ← 入力画面に戻る
                      </button>
                      ...
                      <button
                        ...
                        className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2 px-4 md:px-6 rounded-lg flex items-center space-x-2 text-sm md:text-base transition-all duration-200 focus:ring-4 focus:ring-emerald-300 shadow-md"
                    ...
                    <div className="bg-white rounded-lg shadow-lg p-4">
                      ...
                      >
                        ...
                        {showMatrix ? <ChevronDown size={ICON_SIZES.md} /> : <ChevronRight size={ICON_SIZES.md} />}
                      </div>
                      {showMatrix && (
                              ...
                              {sortedJobs.map((result, index) => {
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <td className="px-3 py-3 text-center text-emerald-600 font-bold">{result.job.fee || '-'}万</td>
                                    {conditions.slice(0, 6).map(cond => (
                                      ...
                                      >
                                        {cond.pass ? (
                                          <CheckCircle className="inline-block text-emerald-600" size={ICON_SIZES.md} />
                                        ) : cond.canRelax ? (
                                          <AlertCircle className="inline-block text-amber-600" size={ICON_SIZES.md} />
                                        ) : (
                                          <XCircle className="inline-block text-red-600" size={ICON_SIZES.md} />
                                        )}
                                      </td>
                                    ))}
                                    <td className="px-3 py-3 text-center text-sm">
                                      {result.isImmediateMatch ? (
                                        ...
                                        <span className="text-amber-600 font-bold">{result.relaxableFailedConditions.length}項目</span>
                                      ) : (
                    ...
                    {/* 電話確認シート */}
                    <div className="bg-white rounded-lg shadow-lg p-4">
                      <h2 className="text-base md:text-lg font-bold text-emerald-600 mb-3 flex items-center">
                        <Phone className="mr-2" size={ICON_SIZES.md} />
                        📞 電話確認シート
                      </h2>
                      ...
                      {immediateMatches.length > 0 && (
                        <div>
                          <div 
                            className="bg-emerald-100 border-l-4 border-emerald-500 p-3 cursor-pointer hover:bg-emerald-200 transition"
                            onClick={() => toggleConditionExpansion('immediate')}
                          >
                            <div className="flex justify-between items-center">
                              <h3 className="text-sm md:text-base font-bold text-emerald-800 flex items-center">
                                <CheckCircle className="mr-2" size={ICON_SIZES.md} />
                                すぐ紹介可能 ({immediateMatches.length}件)
                              </h3>
                              {expandedConditions.has('immediate') ? <ChevronDown size={ICON_SIZES.md} /> : <ChevronRight size={ICON_SIZES.md} />}
                            </div>
                          </div>
                          {expandedConditions.has('immediate') && (
                            <div className="mt-3 space-y-3">
                              {immediateMatches.map(result => (
                                <div 
                                  key={result.job.id} 
                                  className={`bg-emerald-50 border-2 rounded-lg p-3 md:p-4 cursor-pointer transition ${
                                    selectedJobForTracking === result.job.id ? 'border-purple-500 ring-2 ring-purple-300' : 'border-emerald-300'
                                  }`}
                                  onClick={() => setSelectedJobForTracking(result.job.id)}
                                >
                                  <div className="flex justify-between items-start mb-2 flex-wrap gap-2">
                                    <div className="flex-1 min-w-0">
                                      <h4 className="text-sm md:text-base font-bold text-emerald-900 flex items-center">
                                        {result.job.name}
                                  ...
                                  <div className="bg-white rounded p-3 border border-emerald-200">
                                    <div className="font-bold text-emerald-800 text-sm mb-2 flex items-center">
                                      <CheckCircle className="mr-2" size={ICON_SIZES.sm} />
                                      すべての条件クリア:
                                    </div>
                      ...
                      {possibleMatches.length > 0 && (
                        <div className="mt-4">
                          <div 
                            className="bg-amber-100 border-l-4 border-amber-500 p-3 cursor-pointer hover:bg-amber-200 transition"
                            onClick={() => toggleConditionExpansion('possible')}
                          >
                            <div className="flex justify-between items-center">
                              <h3 className="text-sm md:text-base font-bold text-amber-800 flex items-center">
                                <AlertCircle className="mr-2" size={ICON_SIZES.md} />
                                条件確認が必要 ({possibleMatches.length}件)
                              </h3>
                              {expandedConditions.has('possible') ? <ChevronDown size={ICON_SIZES.md} /> : <ChevronRight size={ICON_SIZES.md} />}
                            </div>
                          </div>
                            ...
                            <div className="mt-3 space-y-3">
                              {possibleMatches.map(result => {
                                ...
                                return (
                                  <div 
                                    key={result.job.id} 
                                    className={`bg-amber-50 border-2 rounded-lg p-3 md:p-4 cursor-pointer transition ${
                                      selectedJobForTracking === result.job.id ? 'border-purple-500 ring-2 ring-purple-300' : 'border-amber-300'
                                    }`}
                                    onClick={() => setSelectedJobForTracking(result.job.id)}
                                  >
                                    <div className="flex justify-between items-start mb-2 flex-wrap gap-2">
                                      <div className="flex-1 min-w-0">
                                        <h4 className="text-sm md:text-base font-bold text-amber-900 flex items-center">
                                          {result.job.name}
                                          {selectedJobForTracking === result.job.id && <span className="ml-2 text-purple-600">🎯</span>}
                                        </h4>
                                        <div className="text-xs md:text-sm text-gray-600 mt-1">
                                    ...
                                    <div className="bg-white rounded p-3 border border-amber-200">
                                      <div className="font-bold text-amber-800 text-sm mb-2 flex items-center">
                                        <AlertCircle className="mr-2" size={ICON_SIZES.sm} />
                                        以下の点をご確認ください:
                                      </div>
                                      <div className="space-y-2">
                                        {relaxableConditions.map(cond => {
                                          ...
                                          return (
                                            <div key={cond.id} className={`p-2 rounded transition ${
                                              isChecked ? 'bg-emerald-100 border-2 border-emerald-300' : 'bg-amber-50 border-2 border-amber-200'
                                            }`}>
                                              <div className="flex items-start space-x-2">
                                                <input
                                                  ...
                                                  className="mt-1 focus:ring-2 focus:ring-amber-500 cursor-pointer"
                          ...
                          >
                            <div className="flex justify-between items-center">
                              <h3 className="text-sm md:text-base font-bold text-red-800 flex items-center">
                                <XCircle className="mr-2" size={ICON_SIZES.md} />
                                紹介不可 ({impossibleMatches.length}件)
                              </h3>
                              {expandedConditions.has('impossible') ? <ChevronDown size={ICON_SIZES.md} /> : <ChevronRight size={ICON_SIZES.md} />}
                            </div>
                          </div>
                                ...
                                >
                                  ...
                                  <div className="bg-white rounded p-3 border border-red-200">
                                    <div className="font-bold text-red-800 text-sm mb-2 flex items-center">
                                      <XCircle className="mr-2" size={ICON_SIZES.sm} />
                                      紹介不可:
                                    </div>
                          ...
                          <div className="space-y-2 text-xs">
                            <div className="bg-blue-50 rounded p-2">
                              <div className="font-semibold text-blue-900 flex items-center">
                                <User size={ICON_SIZES.xs} className="mr-1" />
                                求職者:
                              </div>
                            ...
                            {!cond.pass && (
                              <div className={`rounded p-2 ${cond.canRelax ? 'bg-amber-50' : 'bg-red-50'}`}>
                                <div className={`font-semibold flex items-center ${cond.canRelax ? 'text-amber-900' : 'text-red-900'}`}>
                                  {cond.canRelax ? (
                                    <><AlertCircle size={ICON_SIZES.xs} className="mr-1" /> 確認必要</>
                                  ) : (
                                    <><XCircle size={ICON_SIZES.xs} className="mr-1" /> 不適合</>
                                  )}
                                </div>
